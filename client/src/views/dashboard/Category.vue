...
const add = async () => {
    let res = await axios.post("/category/token/add", { name: addCategory.name });
...
const update = async () => {
    let res = await axios.put("/category/token/update", { id: updateCategory.id, name: updateCategory.name });
    if (res.data.code == 200) {
        loadDatas();
        message.info(res.data.msg);
    } else {
...
const deleteCategory = async (category) => {
    ...
    dialog.warning({
        ...
        onPositiveClick: async () => {
            let res = await axios.delete(`/category/token/delete?id=${category.id}`);
            if (res.data.code == 200) {
                loadDatas();
                message.info(res.data.msg);
            } else {
